...
export function CustomSymbolDesignerDialog({ open, language, onClose }: Props) {
  ...
  const saveRegisteredSymbol = () => {
    try {
      ...
      if (shapeCount === 0) {
        throw new Error(isEn ? "The symbol must contain at least one visible vector shape." : "字符至少需要一个可见的矢量单元。");
      }
      const fallback = documentState.ommlFallback?.trim() ?? "";
      if (fallback) {
        if (containsCustomSymbolCommand(fallback)) {
          throw new Error(isEn ? "Word fallback cannot depend on another VisualTeX custom symbol." : "Word fallback 不能依赖其他 VisualTeX 自定义字符。");
        }
      ...
      if (!saved) throw new Error(isEn ? "VisualTeX could not read the registered symbol back." : "VisualTeX 无法重新读取刚注册的字符。");
      ...
      setRegistrationState({
        kind: "success",
        message: existing
          ? isEn ? `Updated \\${saved.command}` : `已更新 \\${saved.command}`
          : isEn ? `Registered \\${saved.command}` : `已注册 \\${saved.command}`,
      });
    } catch (error) {
      ...
      >
        <header className="custom-symbol-designer-header">
          <div>
            ...
            <span>{isEn ? "Compose editable LaTeX vector glyphs on a mathematical baseline." : "在数学基线上重新组合可编辑的 LaTeX 矢量字形。"}</span>
          </div>
          <div>
            <button className="icon-button compact" type="button" data-reset-custom-symbol-designer onClick={reset} aria-label={isEn ? "Reset" : "重置"}><RotateCcw size={15} /></button>
            <button className="icon-button compact" type="button" onClick={onClose} aria-label={isEn ? "Close" : "关闭"}><X size={16} /></button>
          </div>
        </header>
        ...
        <div className="custom-symbol-designer-body">
          <aside className="custom-symbol-designer-sidebar is-materials">
            <section className="custom-symbol-designer-panel is-registered-symbols">
              <header><strong>{isEn ? "Registered symbols" : "已注册字符"}</strong><span>{registeredSymbols.length}</span></header>
                  ...
                  return (
                    <div key={symbol.id} className={`custom-symbol-registered-item${active ? " is-active" : ""}`} data-registered-custom-symbol={symbol.id} data-registered-custom-symbol-command={symbol.command}>
                      ...
                      <div className="custom-symbol-registered-actions">
                        <button type="button" data-duplicate-registered-custom-symbol onClick={() => duplicateRegisteredSymbol(symbol)} title={isEn ? "Duplicate as draft" : "复制为新草稿"}><Copy size={12} /></button>
                        <button type="button" data-delete-registered-custom-symbol className={pendingDelete ? "is-pending-delete" : ""} onClick={() => requestDeleteRegisteredSymbol(symbol)} title={pendingDelete ? isEn ? "Click again below to confirm" : "请在下方再次确认" : isEn ? "Delete registered symbol" : "删除已注册字符"}><Trash2 size={12} /></button>
                      </div>
                      {pendingDelete ? <div className="custom-symbol-delete-warning" data-custom-symbol-delete-warning>
                        <span>{isEn ? `Deleting \\${symbol.command} makes existing formulas using it unresolved.` : `删除 \\${symbol.command} 后，已有公式中的这个命令会变成未解析状态。`}</span>
                        <div>
                          <button type="button" data-confirm-delete-registered-custom-symbol onClick={() => requestDeleteRegisteredSymbol(symbol)}>{isEn ? "Delete" : "确认删除"}</button>
                          <button type="button" data-cancel-delete-registered-custom-symbol onClick={() => setPendingDeleteSymbolId(null)}>{isEn ? "Cancel" : "取消"}</button>
                        </div>
                      </div> : null}
                    </div>
                  );
                })}
                {!registeredSymbols.length ? <div className="custom-symbol-registered-empty">{isEn ? "Registered symbols will appear here for reuse and editing." : "注册后的字符会出现在这里，可再次编辑或复用。"}</div> : null}
              </div>
            </section>
            ...
            <section className="custom-symbol-designer-panel">
              ...
              <div className="custom-symbol-material-input-row">
                ...
                <button type="button" data-add-custom-symbol-material onClick={() => addMaterial()} disabled={!materialLatex.trim()}><Plus size={14} />{isEn ? "Add" : "加入"}</button>
              </div>
              ...
              <div className="custom-symbol-geometry-heading">{isEn ? "Geometry" : "几何图形"}</div>
          ...
          <main className="custom-symbol-designer-stage-column">
            <div className="custom-symbol-designer-stage-toolbar">
              <span>{isEn ? "Output box" : "输出范围"} · {widthEm.toFixed(3)}em × {heightEm.toFixed(3)}em</span>
              <div className="custom-symbol-designer-stage-toolbar-actions">
                ...
                <button type="button" className={showReference ? "is-active" : ""} data-toggle-custom-symbol-reference data-toggle-custom-symbol-reference-alpha aria-pressed={showReference} onClick={() => setShowReference((current) => !current)} title={isEn ? "Show or hide the selected mathematical reference on the same baseline" : "显示或隐藏与当前字符共用数学基线的参考符号"}>{showReference ? <Eye size={12} /> : <EyeOff size={12} />}{showReference ? referencePreset.label : isEn ? "Reference" : "参考"}</button>
                <button type="button" className={`custom-symbol-eraser-tool${eraserMode ? " is-active" : ""}`} data-custom-symbol-eraser-tool aria-pressed={eraserMode} onClick={() => setEraserMode((current) => !current)} title={isEn ? "Erase unwanted portions with a true transparent vector cutout" : "用真正的透明矢量擦除去掉不需要的部分"}><Eraser size={12} />{isEn ? "Eraser" : "橡皮擦"}</button>
                {eraserMode ? <label className="custom-symbol-eraser-size"><span>{isEn ? "Size" : "粗细"}</span><input type="range" min="4" max="240" step="2" value={Math.min(240, eraserSize)} data-custom-symbol-eraser-size onChange={(event) => setEraserSize(Number(event.currentTarget.value))} /><input type="number" min="4" max="600" step="2" value={eraserSize} data-custom-symbol-eraser-size-number aria-label={isEn ? "Precise eraser size" : "精确橡皮擦粗细"} onChange={(event) => { const value = Number(event.currentTarget.value); if (Number.isFinite(value)) setEraserSize(Math.max(4, Math.min(600, value))); }} /><output>{(eraserSize / 1000).toFixed(3)}em</output></label> : null}
              </div>
            </div>
          ...
          <aside className="custom-symbol-designer-sidebar is-inspector">
            ...
            <section className="custom-symbol-designer-panel">
              ...
              <div className="custom-symbol-metric-presets" data-custom-symbol-metric-presets>
                <button type="button" data-metric-preset="standard" onClick={() => applyMetricPreset("standard")}>{isEn ? "Standard" : "标准"}</button>
                <button type="button" data-metric-preset="reference" onClick={() => applyMetricPreset("reference")} disabled={!referenceAsset}>{isEn ? `Match ${referencePreset.label}` : `匹配 ${referencePreset.label}`}</button>
                <button type="button" data-metric-preset="large" onClick={() => applyMetricPreset("large")}>{isEn ? "Large operator" : "大型算子"}</button>
                <button type="button" data-metric-preset="extra" onClick={() => applyMetricPreset("extra")}>{isEn ? "Extra large" : "超大型"}</button>
              </div>
              <div className="custom-symbol-metric-hint">{isEn ? "The large workspace is only for editing. This output box controls the final TeX size and clipping bounds." : "大工作区只用于设计；这里的输出范围才决定最终 TeX 字符尺寸与裁剪边界。"}</div>
            </section>
            ...
            <section className="custom-symbol-designer-panel">
              ...
              {selectedLayer ? <>
                ...
                {selectedLayer.kind === "geometry" && selectedGeometryProperties ? <div className="custom-symbol-geometry-properties" data-custom-symbol-geometry-properties>
                  <div className="custom-symbol-crop-heading"><strong>{selectedLayer.geometryPreset === "eraser" ? isEn ? "Eraser stroke" : "橡皮擦轨迹" : isEn ? "Geometry" : "几何属性"}</strong><span>{selectedLayer.geometryPreset ?? selectedLayer.shape.kind}</span></div>
                  <div className="custom-symbol-designer-number-grid">
                    {selectedLayer.geometryPreset !== "eraser" ? <NumericField label={selectedLayer.geometryPreset === "line" ? isEn ? "Length" : "长度" : selectedLayer.geometryPreset === "circle" ? isEn ? "Diameter" : "直径" : isEn ? "Width" : "宽度"} field="geometry-width" value={selectedGeometryProperties.width} step={10} min={10} max={8000} onChange={(value) => updateSelectedGeometry({ width: value })} /> : null}
                    {selectedLayer.geometryPreset !== "line" && selectedLayer.geometryPreset !== "circle" && selectedLayer.geometryPreset !== "eraser" ? <NumericField label={isEn ? "Height" : "高度"} field="geometry-height" value={selectedGeometryProperties.height} step={10} min={10} max={8000} onChange={(value) => updateSelectedGeometry({ height: value })} /> : null}
                    <NumericField label={selectedLayer.geometryPreset === "eraser" ? isEn ? "Eraser width" : "擦除宽度" : isEn ? "Stroke width" : "线宽"} field="geometry-stroke-width" value={selectedGeometryProperties.strokeWidth} step={2} min={0} max={600} onChange={(value) => updateSelectedGeometry({ strokeWidth: value })} />
                    {selectedLayer.geometryPreset === "rect" ? <NumericField label={isEn ? "Corner radius" : "圆角"} field="geometry-corner-radius" value={selectedGeometryProperties.cornerRadius} step={2} min={0} max={4000} onChange={(value) => updateSelectedGeometry({ cornerRadius: value })} /> : null}
                  </div>
                  {selectedLayer.geometryPreset !== "line" && selectedLayer.geometryPreset !== "arrow" && selectedLayer.geometryPreset !== "arc" && selectedLayer.geometryPreset !== "eraser" ? <label className="custom-symbol-geometry-fill-toggle"><input type="checkbox" data-geometry-fill checked={selectedGeometryProperties.fill} onChange={(event) => updateSelectedGeometry({ fill: event.currentTarget.checked })} /><span>{isEn ? "Filled shape" : "填充图形"}</span></label> : null}
                </div> : null}
                {selectedLayer.kind === "glyph" ? <div className="custom-symbol-crop-controls" data-custom-symbol-crop-controls>
                  <div className="custom-symbol-crop-heading"><strong>{isEn ? "Non-destructive crop" : "非破坏裁剪"}</strong><span>{selectedLayer.clipRect ? isEn ? "Enabled" : "已启用" : isEn ? "Full glyph" : "完整字形"}</span></div>
                  <div className="custom-symbol-crop-presets">
                    <button type="button" data-crop-preset="full" onClick={() => applyClipPreset("full")}>{isEn ? "Full" : "完整"}</button><button type="button" data-crop-preset="top" onClick={() => applyClipPreset("top")}>{isEn ? "Top" : "上"}</button><button type="button" data-crop-preset="middle" onClick={() => applyClipPreset("middle")}>{isEn ? "Middle" : "中"}</button><button type="button" data-crop-preset="bottom" onClick={() => applyClipPreset("bottom")}>{isEn ? "Bottom" : "下"}</button><button type="button" data-crop-preset="left" onClick={() => applyClipPreset("left")}>{isEn ? "Left" : "左"}</button><button type="button" data-crop-preset="center" onClick={() => applyClipPreset("center")}>{isEn ? "Center" : "中列"}</button><button type="button" data-crop-preset="right" onClick={() => applyClipPreset("right")}>{isEn ? "Right" : "右"}</button>
                  </div>
                  ...
                  <div className="custom-symbol-split-actions"><button type="button" data-split-custom-symbol-glyph="horizontal" onClick={() => splitSelectedGlyph("horizontal")}>{isEn ? "Split top / middle / bottom" : "上 / 中 / 下三分"}</button><button type="button" data-split-custom-symbol-glyph="vertical" onClick={() => splitSelectedGlyph("vertical")}>{isEn ? "Split left / center / right" : "左 / 中 / 右三分"}</button></div>
                </div> : null}
            ...
            <section className="custom-symbol-designer-panel is-registration" data-custom-symbol-registration-panel data-registration-dirty={registrationDirty ? "true" : "false"} data-registration-symbol-id={registeredSymbol?.id ?? ""}>
              <header><strong>{isEn ? "Register as a LaTeX command" : "注册为 LaTeX 命令"}</strong><span>{registeredSymbol ? registrationDirty ? isEn ? "Unsaved changes" : "有未保存修改" : isEn ? "Registered" : "已注册" : isEn ? "Draft" : "草稿"}</span></header>
              <label className="custom-symbol-registration-field"><span>{isEn ? "Symbol name" : "字符名称"}</span><input type="text" value={documentState.name} data-custom-symbol-name-input onChange={(event) => { const value = event.currentTarget.value; setDocumentState((current) => ({ ...current, name: value })); }} placeholder={isEn ? "My custom symbol" : "我的自定义字符"} /></label>
              <label className="custom-symbol-registration-field"><span>{isEn ? "Command" : "命令"}</span><input type="text" value={documentState.command} data-custom-symbol-command-input spellCheck={false} onChange={(event) => { const value = event.currentTarget.value; setDocumentState((current) => ({ ...current, command: value })); }} placeholder="\\selfdefa" /></label>
              <label className="custom-symbol-registration-field"><span>{isEn ? "Math role" : "数学类型"}</span><select value={documentState.role} data-custom-symbol-role-select onChange={(event) => { const role = event.currentTarget.value as CustomSymbolMathRole; setDocumentState((current) => ({ ...current, role })); }}>{customSymbolMathRoles.map((role) => <option key={role} value={role}>{mathRoleLabel(role)}</option>)}</select></label>
              {documentState.role === "operator" ? <label className="custom-symbol-registration-field"><span>{isEn ? "Limits" : "上下限行为"}</span><select value={documentState.limitsBehavior} data-custom-symbol-limits-select onChange={(event) => { const limitsBehavior = event.currentTarget.value as CustomSymbolLimitsBehavior; setDocumentState((current) => ({ ...current, limitsBehavior })); }}>{customSymbolLimitsBehaviors.map((behavior) => <option key={behavior} value={behavior}>{limitsLabel(behavior)}</option>)}</select></label> : null}
              <label className="custom-symbol-registration-field"><span>{isEn ? "Word OMML fallback (optional)" : "Word OMML fallback（可选）"}</span><input type="text" value={documentState.ommlFallback ?? ""} data-custom-symbol-omml-fallback-input spellCheck={false} onChange={(event) => { const value = event.currentTarget.value; setDocumentState((current) => ({ ...current, ommlFallback: value || null })); }} placeholder="\\approx" /></label>
              <button type="button" className="custom-symbol-register-button" data-register-custom-symbol onClick={saveRegisteredSymbol} disabled={!documentState.name.trim() || !documentState.command.trim() || shapeCount === 0}>{registeredSymbol ? isEn ? "Update registered symbol" : "更新已注册字符" : isEn ? "Register symbol" : "注册字符"}</button>
        ...
        <footer className="custom-symbol-designer-footer"><span>{documentState.layers.length} {isEn ? "layers" : "图层"} · {shapeCount} {isEn ? "shapes" : "矢量单元"}</span><button type="button" onClick={onClose}>{isEn ? "Close" : "关闭"}</button></footer>
      </section>
    </div>,
    document.body,
  );
