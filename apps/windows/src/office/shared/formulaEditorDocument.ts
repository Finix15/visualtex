...
const structuredRenderFormats = new Set<LatexCodeFormat>([
  "equation",
  "equation-star",
  "align",
  "align-star",
...
/**
 * Source/copy formats such as `$...$`, `\\(...\\)` and `\\[...\\]` are not
 * TeX input that MathJax expects after it is already in math mode. Preserve
 * those wrappers in serialized metadata, but render the logical formula body.
 * Structured environments keep their canonical environment so first import
 * and subsequent Office edits produce the same geometry and alignment.
 */
export function serializeFormulaEditorRenderDocument(
  document: FormulaEditorDocument,
) {
  if (!structuredRenderFormats.has(document.codeFormat)) {
    return formatLatexLines(
      document.lines.map((line) => line.latex),
      "raw",
    );
  }
  const canonical = serializeFormulaEditorDocument(document);
  if (document.codeFormat !== "aligned") return canonical;
  return canonical
    .replace(/^\s*\\\[\s*/, "")
    .replace(/\s*\\\]\s*$/, "")
    .trim();
