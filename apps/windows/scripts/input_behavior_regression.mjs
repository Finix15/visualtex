      ...
      try {
        await evaluate(`new Promise((resolve, reject) => {
          ...
          const done = () => {
            ...
            if (Date.now() - startedAt > 5000) {
              reject(new Error("Timed out waiting for the formula field after reload"));
              return;
            }
            setTimeout(done, 30);
          };
        ...
        await evaluate(`new Promise((resolve, reject) => {
          ...
          const done = () => {
            ...
            if (Date.now() - startedAt > 5000) {
              reject(new Error("Timed out waiting for the formula field after recovery navigation"));
              return;
            }
            setTimeout(done, 30);
          };
