...
assert.ok(officeDialogApp.includes("OFFICE_FONT_SIZE_OPTIONS"));
assert.ok(officeDialogApp.includes("officePointFontSizeOptions(officeFontSizePt)"));
assert.ok(officeDialogApp.includes('label={isEn ? "Point sizes" : "Kích thước điểm"}'));
...
assert.ok(officeDialogApp.includes("Áp dụng và đóng (Ctrl+S)"));
...
assert.ok(formulaToolbar.includes("Thêm vào chung"));
...
assert.ok(documentImportApp.includes("Xem trước cấu trúc từ"));
...
assert.ok(documentImportApp.includes("Nhập .tex / .md"));
...
assert.ok(documentImportApp.includes("Đã chỉnh sửa"));
...
assert.ok(wordVstoService.includes("The active Word document changed while the VisualTeX editor was open."));
assert.ok(powerpointVstoService.includes("The active PowerPoint presentation changed while the VisualTeX editor was open."));
...
assert.ok(certificateUninstaller.includes('foreach ($storeName in @("Root", "TrustedPeople"))'));
assert.ok(certificateUninstaller.includes("CertificateThumbprint"));
assert.ok(certificateUninstaller.includes("CertificateCleanupPending"));
assert.ok(certificateUninstaller.includes('Remove-Item -LiteralPath "Cert:\\CurrentUser\\$storeName\\$thumbprint"'));
assert.ok(certificateUninstaller.includes("TimeoutSeconds"));
assert.ok(uninstallVsto.includes("certificate cleanup failed. Uninstall will continue"));
...
assert.ok(installerHooks.includes("self-signed certificate for HTTPS communication restricted to localhost/127.0.0.1"));
...
assert.ok(installerHooks.includes("tài liệu Office chưa lưu có thể bị mất"));
assert.ok(installerHooks.includes("quay lại trang trước"));
