...
const runtimeHostProbeSource = await readFile("scripts/test_windows_office_host_probe.ps1", "utf8");
...
assert(windowsSettingsSource.includes('isEn ? "Open Word" : "Mở Word"'));
assert(!windowsSettingsSource.includes('isEn ? "Open Word" : "Lời mở"'));
assert(windowsSettingsSource.includes('run("word", "open_word")'));
...
assert(windowsBundleSource.includes('"../scripts/test_windows_office_host_probe.ps1"'));
...
assert(runtimeTestSource.includes("Invoke-HiddenOfficeComAddInProbe"));
...
assert(runtimeTestSource.includes('startupMode = "hidden-com-worker"'));
assert(runtimeTestSource.includes("RuntimeVerificationPending"));
assert(runtimeTestSource.includes("WorkerTimeoutSeconds = 25"));
assert(runtimeHostProbeSource.includes("New-Object -ComObject Word.Application"));
assert(runtimeHostProbeSource.includes("New-Object -ComObject PowerPoint.Application"));
assert(runtimeHostProbeSource.includes("$application.Visible = $false"));
assert(runtimeHostProbeSource.includes("$document.Close(0)"));
assert(runtimeHostProbeSource.includes("$application.Quit()"));
...
assert(removeCertificateSource.includes('foreach ($storeName in @("Root", "TrustedPeople"))'));
assert(removeCertificateSource.includes("CertificateThumbprint"));
assert(removeCertificateSource.includes("CertificateCleanupPending"));
assert(removeCertificateSource.includes("Remove-Item -LiteralPath \"Cert:\\CurrentUser\\$storeName\\$thumbprint\""));
...
assert(hooksSource.includes("self-signed certificate for HTTPS communication restricted to localhost/127.0.0.1"));
