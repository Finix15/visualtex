...
function Assert-OfficeAddinRegistration {
    ...
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $subKey "FriendlyName" "VisualTeX" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $subKey "Description" $ExpectedDescription $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $subKey "LoadBehavior" 3 $Architecture | Out-Null
    $manifest = Get-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $subKey "Manifest" $Architecture -Optional
    $codeBase = Get-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $subKey "CodeBase" $Architecture -Optional
    $location = if (-not [string]::IsNullOrWhiteSpace([string]$manifest)) { [string]$manifest } else { [string]$codeBase }
    if ([string]::IsNullOrWhiteSpace($location)) {
        throw "$HostName add-in registration does not contain Manifest or CodeBase: HKCU\$subKey"
    }
...
function Assert-ComRegistration {
    ...
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\$ProgId\CLSID" "(default)" $Clsid $Architecture | Out-Null
    $classKey = "Software\Classes\CLSID\$Clsid"
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "$classKey\InprocServer32" "(default)" "mscoree.dll" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "$classKey\InprocServer32" "Class" $ClassName $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "$classKey\InprocServer32" "Assembly" $assemblyName.FullName $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "$classKey\InprocServer32" "RuntimeVersion" $runtimeVersion $Architecture | Out-Null
    $codeBase = [string](Get-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "$classKey\InprocServer32" "CodeBase" $Architecture)
    ...
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $versionKey "Class" $ClassName $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $versionKey "Assembly" $assemblyName.FullName $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $versionKey "RuntimeVersion" $runtimeVersion $Architecture | Out-Null
    $versionCodeBase = [string](Get-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) $versionKey "CodeBase" $Architecture)
    ...
    foreach ($requiredKey in @(
        ...
        if (-not (Test-RegistryKey ([Microsoft.Win32.RegistryHive]::CurrentUser) $requiredKey $Architecture)) {
...
function Assert-NativeOleRegistration([string]$InstallRoot, [string]$Architecture) {
    ...
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\VisualTeX.Formula.1\CLSID" "(default)" $clsid $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\VisualTeX.Formula\CurVer" "(default)" "VisualTeX.Formula.1" $Architecture | Out-Null
    $localServer = [string](Get-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\LocalServer32" "(default)" $Architecture)
    ...
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\LocalServer32" "ServerExecutable" $server $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\DataFormats\GetSet\0" "(default)" "14,1,64,1" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\DataFormats\GetSet\1" "(default)" "3,1,32,1" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\DataFormats\GetSet\2" "(default)" "PNG,1,1,1" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\ProgID" "(default)" "VisualTeX.Formula.1" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\CLSID\$clsid\VersionIndependentProgID" "(default)" "VisualTeX.Formula" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\Interface\$iid\ProxyStubClsid32" "(default)" "{00020424-0000-0000-C000-000000000046}" $Architecture | Out-Null
    Assert-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\Interface\$iid\TypeLib" "(default)" $libid $Architecture | Out-Null
    $typeLibraryPlatform = if ($Architecture -eq "x86") { "win32" } else { "win64" }
    $typeLibrary = [string](Get-RegistryValue ([Microsoft.Win32.RegistryHive]::CurrentUser) "Software\Classes\TypeLib\$libid\1.0\0\$typeLibraryPlatform" "(default)" $Architecture)
...
try {
    ...
    Add-DiagnosticCheck "Static installation verification" $true "MSI, files, hashes, Office registrations, managed COM classes and native OLE LocalServer passed." "static-install"
    Write-Host "Static files and registry installation verified successfully."

    $runtimeScript = Join-Path $PSScriptRoot "test_windows_office_runtime.ps1"
    if (-not (Test-Path -LiteralPath $runtimeScript -PathType Leaf)) {
        throw "Companion runtime verification script is missing after static installation: $runtimeScript"
    }
    $runtimeReport = Join-Path $logRoot "companion-runtime-$stamp.json"
    try {
        & $runtimeScript `
            -OfficePlatform $script:resolvedOfficePlatform `
            -VisualTeXPath $script:resolvedVisualTeXPath `
        ...
        $script:runtimeVerified = $true
        Add-DiagnosticCheck "Companion runtime verification" $true "Process, port, HTTPS, certificate and protocol passed. Report=$runtimeReport" "runtime"
        Write-DiagnosticReport $true $true $true
        Write-Host "installed and runtime-verified: VisualTeX VSTO Ribbon, native OLE LocalServer and companion runtime passed."
        Write-Warning "Office applications were not launched; run test_windows_office_runtime.ps1 without -CompanionOnly to verify Word and PowerPoint COMAddIn.Connect."
    } catch {
        $runtimeFailure = $_.Exception.Message
        Add-DiagnosticCheck "Companion runtime verification" $false $runtimeFailure "runtime"
        New-ItemProperty -LiteralPath $modeKey -Name "OfficeRuntimeVerified" -PropertyType DWord -Value 0 -Force | Out-Null
        New-ItemProperty -LiteralPath $modeKey -Name "LastRuntimeError" -PropertyType String -Value $runtimeFailure -Force | Out-Null
        Write-DiagnosticReport $false $true $false $runtimeFailure
        Write-Warning "Office add-ins installed successfully, but local companion runtime verification failed."
