...
expectIncludes(wordAdapter, '"word-structured-document-import-20260729-r53"', "Word health must identify the structured document-import, SVG baseline, and equation-number revision");
...
expectIncludes(wordEvents, "VTIsVisualTeXNativeSelection", "Word native double-click events must recognize VisualTeX OMML formulas without affecting ordinary objects");
...
expectIncludes(wordAdapter, "VTFindNativeFormulaBookmark(Selection.Range, False)", "The global Word double-click macro must probe native formulas without raising on ordinary text or blank space");
expectIncludes(wordAdapter, "If nativeBookmark Is Nothing Then Exit Sub", "A Word double-click without a VisualTeX target must be a strict no-op");
...
expectIncludes(nativeInteraction, "push_word_edit_selected", "The Word compatibility branch must preserve image editing when the VBA event sink is unavailable");
...
expectIncludes(packager, "word-structured-document-import-20260729-r53", "Packaging must reject a Word DOTM that lacks the structured document-import and equation-layout revision");
