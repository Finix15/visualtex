...
3. Run **Debug → Compile VBAProject**. Confirm `VTWordEvents` compiles, `AutoExec` initializes the application event sink, and the compiled project contains `word-document-import-20260729-r52`, `VTWordRibbonDocumentImport`, `VisualTeX_InsertLatexMarkdownDocument`, `App_WindowSelectionChange`, `VTWordRibbonApplyImageFontSizePreset`, and `VTRefreshNumberedImageFormulaFontLayout`.
