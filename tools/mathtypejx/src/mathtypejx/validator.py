...
def _extract_omml_tokens(root) -> list[str]:
    ...
    # Word stores stretchy delimiters structurally on m:d rather than in
    # m:t runs.  Missing m:begChr/m:endChr means the OMML defaults "(" and
    # ")"; an explicit empty m:val suppresses that side (for cases, etc.).
    tokens.extend(_extract_omml_delimiter_tokens(root))
    return tokens


def _extract_omml_delimiter_tokens(root) -> list[str]:
    tokens = []
    for delim in root.iter():
        if not isinstance(delim.tag, str):
            continue
        qname = etree.QName(delim)
        if qname.namespace != OMML_NS or qname.localname != "d":
            continue
        ...
        for child in delim:
            ...
            if child_qname.namespace != OMML_NS or child_qname.localname != "dPr":
                continue
            for prop in child:
                if not isinstance(prop.tag, str):
                    continue
                prop_qname = etree.QName(prop)
                if prop_qname.namespace != OMML_NS:
                    continue
                value = prop.get(f"{{{OMML_NS}}}val", "")
                if prop_qname.localname == "begChr":
                    begin = value
                elif prop_qname.localname == "endChr":
                    end = value
        if begin:
            tokens.append(_normalize_token(begin))
        if end:
            tokens.append(_normalize_token(end))
    return tokens
